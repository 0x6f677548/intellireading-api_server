...
#creates a zip file for the current directory in the _releases folder, named after the folder
...
#usage example: create_server_release_zip.sh <version>
...
if [ -z "$VERSION" ]; then
  ...
  echo "Usage: create_server_release_zip.sh <version>"
  exit 1
fi
...
echo "Creating server release zip $VERSION in $(pwd)/.releases/$VERSION"
...
rsync -av --delete --exclude-from=rsync-exclusions "$ROOT/../" "$TEMP"

#change to the temp folder to avoid including the temp folder in the zip file
cd "$TEMP" || exit 1

#replace @@API_SERVER_VERSION@@ in the .env.prod file with the version
sed -i -e "s/@@API_SERVER_VERSION@@/$VERSION/g" .env.prod

#zip the temp folder and move it to the releases folder
zip -r -m "$RELEASES/intellireading-server-$VERSION.zip" ./
